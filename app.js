...
const seed = () => ({
  ...
  drafts: {},
  ui: { selectedCompany: "company-1", leadFilter: "all", taskFilter: "today", crmType: "all" }
});
...
function allContactMethods(company) {
  const methods = [];
  ...
  return unique.sort((a,b) => { const ai = order.indexOf(a.id), bi = order.indexOf(b.id); return (ai < 0 ? 999 : ai) - (bi < 0 ? 999 : bi); });
}
function normalizedContactOrder(company) { const ids = allContactMethods({ ...company, contactMethodOrder: [] }).map(method => method.id); return [...(company.contactMethodOrder || []).filter(id => ids.includes(id)), ...ids.filter(id => !(company.contactMethodOrder || []).includes(id))]; }
function moveContactMethod(company, methodId, direction) { const order = normalizedContactOrder(company); const index = order.indexOf(methodId); const target = index + direction; if (index < 0 || target < 0 || target >= order.length) return; [order[index], order[target]] = [order[target], order[index]]; company.contactMethodOrder = order; }
...
function methodOutreachCount(company, method) {
  const sameTypeMethods = allContactMethods(company).filter(item => item.type === method.type);
...
function deriveCompanyTask(company) {
  if (hasCompanyResponse(company) || ["Interested", "Paused", "Do not contact"].includes(company.stage)) return null;
  ...
  const eligible = allContactMethods(company).map((method,index) => ({ ...method, index, count: methodOutreachCount(company, method) })).filter(method => method.count < maxPerMethod);
  if (!eligible.length) return null;
  let method = latest?.methodId ? eligible.find(item => item.id === latest.methodId) : null;
  // Older history entries may not identify a specific method, so match their channel.
  // Once a specifically identified method is exhausted, ranking—not prior usage—chooses the next method.
  if (!method && latest && !latest.methodId) method = eligible.find(item => item.type === activityChannel(latest));
  if (!method) method = [...eligible].sort((a,b) => a.index - b.index)[0];
...
function syncCompanyScheduleFromHistory(company) {
  ...
  if (!hasCompanyResponse(company) && !["Interested", "Paused", "Do not contact"].includes(company.stage)) company.stage = !outbound.length ? "New" : derived?.due <= today() ? "Follow-up due" : derived ? "Waiting for reply" : "Contact exhausted";
}
...
function renderCRM() {
  ...
  const query = $("#globalSearch").value.trim().toLowerCase();
  const companies = state.companies.filter(c => (state.ui.crmType === "all" || c.type === state.ui.crmType) && (!query || `${c.company} ${c.location} ${c.type}`.toLowerCase().includes(query)));
  $("#companyList").innerHTML = companies.length ? companies.map(c => {
    ...
    return `<button class="company-row ${c.id === state.ui.selectedCompany ? "active" : ""}" data-company-id="${c.id}">${typeIcon(c.type)}<span class="company-row-main"><strong>${esc(c.company)}</strong><small>${esc(c.location)} · ${esc(c.type)}</small></span><span class="company-row-meta"><strong class="${dueTask?.due < today() ? "overdue-label" : ""}">${esc(statusLabel)}</strong><small>${outreachCount} outreach${outreachCount === 1 ? "" : "es"}</small></span></button>`;
...
function renderCompanyDetail() {
  ...
  <section class="detail-section"><h4>Ranked contact methods</h4>${methods.length ? methods.map((m,index) => `<div class="contact-method ranked"><span class="contact-rank">${index + 1}</span><span class="method-icon">${m.icon}</span><span><strong>${esc(m.label)}</strong><small>${esc(m.value)}</small></span><span class="primary-badge">${methodOutreachCount(company, m)} / ${state.settings.attemptThreshold} used</span><span class="rank-controls"><button class="rank-button" data-rank-method="${esc(m.id)}" data-rank-direction="-1" title="Move up" ${index === 0 ? "disabled" : ""}>↑</button><button class="rank-button" data-rank-method="${esc(m.id)}" data-rank-direction="1" title="Move down" ${index === methods.length - 1 ? "disabled" : ""}>↓</button></span></div>`).join("") : `<p class="metric-note">No contact methods stored.</p>`}</section>
...
$("#companyDetail").addEventListener("click", e => {
  ...
  if (rankButton) { const company = state.companies.find(c => c.id === state.ui.selectedCompany); if (!company) return; moveContactMethod(company, rankButton.dataset.rankMethod, Number(rankButton.dataset.rankDirection)); notify("Contact-method ranking updated"); renderAll(); return; }
